...
int binexp(int a, int b, int m)	// binary exponentiation modulo to get (a ^ b) % m in logarithmic time
{
    a %= m;
    int res = 1;
    while (b > 0)
...
int main()
{
	...
	for(i = 0; i < 8; ++i)
	{
		...
		for(j = 0; j < len; ++j)
		{
			int curr = (final[j] * binexp(10, j, primes[i])) % primes[i];	// finding the remainder of the numbers with the primes generated using modular arithmetic
			rem = (rem + curr) % primes[i];
		}
		for(j = len - 1; j >= 0; --j)
			printf("%d", final[j]);
