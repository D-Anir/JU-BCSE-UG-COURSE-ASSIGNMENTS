...
// animation delay
void MainWindow::animationDelay()
{
    int sleep = ui->delay->value() * 10;
...
// function to illuminate a point on the grid
...
void MainWindow::on_setGridButton_clicked()
{
    ...
    for(int i=0;i<img.width();i+=gridsize)
    {
        ...
        {
            img.setPixel(i,j,qRgb(3, 202, 252));
            img.setPixel(j,i,qRgb(3, 202, 252));
        }
    }
    ui->frame->setPixmap(QPixmap::fromImage(img));
}

// displays the X and Y axes
void MainWindow::on_showAxes_clicked()
{
        for(int j=0;j<=ui->frame->width();j+=gridsize)
        {
            point(img.width()/2,j, 150);
        }
        for(int i=0;i<=ui->frame->height();i+=gridsize)
        {
            point(i,img.height()/2, 150);
        }
}

// resets the entire grid frame
...
// resets the current plots
void MainWindow::on_resetUtil_clicked()
{
    on_setGridButton_clicked();
    on_showAxes_clicked();
}

// shows current position of cursor
...
// stores the pixel coordinate where mouse is pressed
...
void MainWindow::DDAline(int r, int g, int b)
{
    ...
    time_point<std::chrono::system_clock> start2, end2;
    ...
    auto time_taken1 = duration_cast<microseconds>(stop1 - start1);
    auto time_taken2 = duration_cast<microseconds>(start2-end2);

    ui->time_taken->setText(QString::number(time_taken1.count() - time_taken2.count()) + "ms");
}
...
void MainWindow::on_polarCircle_clicked()
{
    ...
    while(angle <= 360){
        animationDelay();
        x = r*cos(PI/180*angle);
        y = r*sin(PI/180*angle);
        int xc = x*gridsize, yc = y*gridsize;
        point(x0+xc, y0+yc);
        angle = angle + 1.0/r;
    }
    end2 = high_resolution_clock::now();
...
void MainWindow::on_draw_ellipse_bresenham_clicked()
{
    auto start1 = high_resolution_clock::now();
    time_point<std::chrono::system_clock> start2, end2;
    ...
    p1.setX(ui->frame->x);
    p1.setY(ui->frame->y);
    int rx = ui->ellipse_major_axis->value();
    int ry = ui->ellipse_minor_axis->value();
    int x_centre=p1.x();
    int y_centre=p1.y();
    int k = gridsize;
...
void MainWindow::on_boundaryFill_clicked()
{
    ...
    point(x,y,0,0,0);
    boundaryfillutil(x,y,qRgb(255,255,0),150,150,150);
}
...
void MainWindow::on_floodFill_clicked()
{
    ...
    y = y*gridsize + gridsize/2;
    point(x,y,0,0,0);
    floodfillutil(x,y,150,150,150);
}


void MainWindow::on_draw_ellipse_polar_clicked()
{
    auto start1 = high_resolution_clock::now();
    time_point<std::chrono::system_clock> start2, end2;
    ...
    while(angle <= 360){
        animationDelay();
        x = rx*cos(PI/180*angle);
        y = ry*sin(PI/180*angle);
        int xc = x*gridsize, yc = y*gridsize;
        point(x0+xc, y0+yc);
        angle = angle + 1.0/(rx+ry);
    }
    end2 = high_resolution_clock::now();
...
void MainWindow::on_setVertex_clicked()
{
    ...
    if(i>=2)
    {
        //storage of edges in edge table.
        storeEdgeInTable(vertex_list[i-2].first, vertex_list[i-2].second, vertex_list[i-1].first, vertex_list[i-1].second);
...
{
    int angle = ui->rotationAngle->value();
...
void MainWindow::on_reflectionX_clicked()
{
    int rx, ry;
    rx = 1, ry = -1;
    std::vector<std::pair<int,int>> old_vertex = vertex_list;
...
void MainWindow::on_reflectionY_clicked()
{
    int rx, ry;
    rx = -1, ry = 1;
    std::vector<std::pair<int,int>> old_vertex = vertex_list;
...
void MainWindow::on_reflectionO_clicked()
{
    int rx, ry;
    rx = -1, ry = -1;
    std::vector<std::pair<int,int>> old_vertex = vertex_list;
...
void MainWindow::on_lineClippping_clicked()
{
    DDAline(0,0,0);
