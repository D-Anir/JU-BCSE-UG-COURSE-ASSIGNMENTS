...
def start_simulation(table, bytes):
    ...
    for _ in range(const.total_receiver_number):
        read_head, write_head = multiprocessing.Pipe()
        read_from_sender_to_channel.append(read_head)     # goes to channel
        write_from_sender_to_channel.append(write_head)   # goes to channel
    ...
    for i in range(const.total_sender_number):
        sender = Sender(i, table[i], write_from_sender_to_channel[i], bytes)
...
if __name__ == "__main__":
    ...
    bytes = int(input("Enter Length of Input Word: "))
    ...
    start_simulation(wls_table, bytes)
