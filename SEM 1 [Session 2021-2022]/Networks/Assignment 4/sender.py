...
class Sender:
    ...
    def __init__(self, name, walsh_code, sender_to_channel, bytes):
        self.name               = name
    ...
    def open_file(self, sender):
        # Generate a random input string of size self.bytes
        random_input = ''.join(random.choices(string.ascii_uppercase, k = self.bytes))

        '''Opens file in read mode and returns file-pointer-object'''
        try:
            file_name = const.input_file_path + 'input' + str(sender+1) + '.txt'
            fptr = open(file_name, 'w', encoding='utf-8')
            fptr.write(random_input)
    ...
    def send_data(self):
        ...
        with open('textfiles/analysis.txt', 'a+', encoding='utf-8') as rep_file:
            rep_file.write("\n\n********** {} SENDER-{} STATS **********\n".format(curr_datetime, self.name+1) + \
                            ...
                            "--> Throughput: {} bps\n".format(round(const.total_sender_number*self.bytes/self.total_delay, 3)) + \
                            "********************************************************\n\n" + '\n')

        # darr.append(self.total_delay)
        # tarr.append(round(const.total_sender_number*self.bytes/self.total_delay, 3))
        # if len(darr) == const.total_sender_number:
        #     with open('textfiles/analysis.txt', 'a+', encoding='utf-8') as rep_file:
        #         average1 = const.Average(darr)
        #         average2 = const.Average(tarr)
        #         rep_file.write("\n{}\n".format(round(average1, 3)))
        #         rep_file.write("{}\n".format(round(average2, 3)))
